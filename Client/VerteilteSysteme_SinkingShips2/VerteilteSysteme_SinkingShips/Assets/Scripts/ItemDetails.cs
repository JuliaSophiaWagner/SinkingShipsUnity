...
public class ItemDetails : MonoBehaviour
{
    ...
    public void InitializeHistoryList(List<History> histories)
    {
        ...
        for (int i = 0; i < this.History.Count; i++)
        {
            ...
            item.transform.Find("Point1").GetComponent<TextMeshProUGUI>().SetText(this.History.ElementAt(i).FirstPlayerPoints.ToString());
            item.transform.Find("Point2").GetComponent<TextMeshProUGUI>().SetText(this.History.ElementAt(i).SecondPlayerPoints.ToString());
            item.SetActive(true);
        }
    }
}